...
ssize_t from_cache_to_shared_memory(int res_msqid, ResMsgBuf* res_msgbuf, int req_msqid, int rereq_msqid,
                                    int fildes, int segid, size_t segment_size) {
  ...
  do {
    ...
    printf("Progress for segid: %d -- pid: %d -- bytes_transferred: %zu -- total length: %zu\n", segid,
           (int)res_msgbuf->mtype, bytes_transferred, file_len);

    if (bytes_transferred < file_len) {
      if (msgrcv(rereq_msqid, &req_msgbuf, sizeof(req_msgbuf.req_message), res_msgbuf->mtype, 0) < 0) {
