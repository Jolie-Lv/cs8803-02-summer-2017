...
extern void handle_with_curl_init();
extern void clean_up_curl();
...
ssize_t handle_request(gfcontext_t *ctx, char *path, void* arg) {
  char url[strlen(server) + strlen(path)];
  strcpy(url, server);
  strcat(url, path);

  return handle_with_curl(ctx, url, arg);
}
...
int main(int argc, char **argv) {
  ...
  gfserver_setopt(&gfs, GFS_WORKER_FUNC, handle_request);
